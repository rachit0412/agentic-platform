...
This is a standalone integration test script (not pytest-compatible).
Run directly: python tests/test_workspace_rbac.py
"""
...
def main():
    ...
    # Members
    m = requests.post(
        f"{BASE}/workspaces/{ENG_ID}/members",
    ...
    eng_mcps = [
        ...
        for m in requests.get(
            f"{BASE}/mcp/servers", headers={"x-workspace-id": ENG_ID}
        )
        .json()
        .get("servers", [])
    ]
    ...
    res_mcps = [
        m["name"]
        for m in requests.get(
            f"{BASE}/mcp/servers", headers={"x-workspace-id": RES_ID}
        )
        .json()
        .get("servers", [])
    ]
    ...
    eng_tools = [
        ...
        for t in requests.get(
            f"{BASE}/custom-tools", headers={"x-workspace-id": ENG_ID}
        )
        .json()
        .get("tools", [])
    ]
    ...
    res_tools = [
        t["name"]
        for t in requests.get(
            f"{BASE}/custom-tools", headers={"x-workspace-id": RES_ID}
        )
        .json()
        .get("tools", [])
    ]
