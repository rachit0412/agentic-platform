...
# Create workspaces
eng = requests.post(
    f"{BASE}/workspaces", json={"name": "Test-Eng", "description": "Engineering"}
).json()
test("Create workspace returns id", "id" in eng, str(eng))

res = requests.post(
    f"{BASE}/workspaces", json={"name": "Test-Research", "description": "Research"}
).json()
...
# Update workspace
upd = requests.put(
    f"{BASE}/workspaces/{ENG_ID}", json={"description": "Updated eng"}
).json()
test("Update workspace", upd.get("description") == "Updated eng")

# Members
m = requests.post(
    f"{BASE}/workspaces/{ENG_ID}/members", json={"user_id": "bob", "role": "member"}
).json()
...
gs = requests.post(
    ...
    headers={"x-workspace-id": "default", "x-user-role": "admin"},
).json()
...
es = requests.post(
    ...
    headers={"x-workspace-id": ENG_ID, "x-user-role": "member"},
).json()
test(
    "Create eng skill",
    es.get("scope") == "workspace" and es.get("workspace_id") == ENG_ID,
)
...
rs = requests.post(
    ...
    headers={"x-workspace-id": RES_ID, "x-user-role": "member"},
).json()
test(
    "Create research skill",
    rs.get("scope") == "workspace" and rs.get("workspace_id") == RES_ID,
)

# List from Engineering: should see Global + Eng, NOT Research
eng_skills = (
    requests.get(f"{BASE}/skills", headers={"x-workspace-id": ENG_ID})
    .json()
    .get("skills", [])
)
...
res_skills = (
    requests.get(f"{BASE}/skills", headers={"x-workspace-id": RES_ID})
    .json()
    .get("skills", [])
)
...
ea = requests.post(
    ...
    headers={"x-workspace-id": ENG_ID},
).json()
test(
    "Create eng agent",
    ea.get("scope") == "workspace" and ea.get("workspace_id") == ENG_ID,
)

ra = requests.post(
    f"{BASE}/agents",
    json={"name": "ResBot", "description": "Res agent", "scope": "workspace"},
    headers={"x-workspace-id": RES_ID},
).json()
test("Create research agent", ra.get("scope") == "workspace")

eng_agents = [
    a["name"]
    for a in requests.get(f"{BASE}/agents", headers={"x-workspace-id": ENG_ID})
    .json()
    .get("agents", [])
]
...
res_agents = [
    a["name"]
    for a in requests.get(f"{BASE}/agents", headers={"x-workspace-id": RES_ID})
    .json()
    .get("agents", [])
]
...
eng_mcps = [
    m["name"]
    for m in requests.get(f"{BASE}/mcp/servers", headers={"x-workspace-id": ENG_ID})
    .json()
    .get("servers", [])
]
...
res_mcps = [
    m["name"]
    for m in requests.get(f"{BASE}/mcp/servers", headers={"x-workspace-id": RES_ID})
    .json()
    .get("servers", [])
]
...
eng_prompts = [
    p["name"]
    for p in requests.get(f"{BASE}/prompts", headers={"x-workspace-id": ENG_ID})
    .json()
    .get("prompts", [])
]
...
res_prompts = [
    p["name"]
    for p in requests.get(f"{BASE}/prompts", headers={"x-workspace-id": RES_ID})
    .json()
    .get("prompts", [])
]
...
eng_tools = [
    t["name"]
    for t in requests.get(f"{BASE}/custom-tools", headers={"x-workspace-id": ENG_ID})
    .json()
    .get("tools", [])
]
...
res_tools = [
    t["name"]
    for t in requests.get(f"{BASE}/custom-tools", headers={"x-workspace-id": RES_ID})
    .json()
    .get("tools", [])
]
...
non_admin_skill = requests.post(
    f"{BASE}/skills",
    json={
        "name": "ShouldBeWorkspace",
        "description": "Member tries global",
        "scope": "global",
    },
    headers={"x-workspace-id": ENG_ID, "x-user-role": "member"},
).json()
test(
    "Member requesting global gets workspace",
    non_admin_skill.get("scope") == "workspace",
    f"Got scope={non_admin_skill.get('scope')}",
)

admin_skill = requests.post(
    f"{BASE}/skills",
    json={
        "name": "ShouldBeGlobal",
        "description": "Admin creates global",
        "scope": "global",
    },
    headers={"x-workspace-id": "default", "x-user-role": "admin"},
).json()
test(
    "Admin requesting global gets global",
    admin_skill.get("scope") == "global",
...
test(
    ...
    cant_del.get("error") is not None or cant_del.get("deleted") == False,
)
...
for sid in [
    ...
    es.get("id"),
    rs.get("id"),
    non_admin_skill.get("id"),
    admin_skill.get("id"),
]:
    if sid:
        requests.delete(f"{BASE}/skills/{sid}")
for aid in [ga.get("id"), ea.get("id"), ra.get("id")]:
    if aid:
        requests.delete(f"{BASE}/agents/{aid}")
for mid in [gm.get("id"), em.get("id")]:
    if mid:
        requests.delete(f"{BASE}/mcp/servers/{mid}")
for pid in [gp.get("id"), ep.get("id")]:
    if pid:
        requests.delete(f"{BASE}/prompts/{pid}")
for tid in [gt.get("id"), et.get("id")]:
    if tid:
        requests.delete(f"{BASE}/custom-tools/{tid}")
