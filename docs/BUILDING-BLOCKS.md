...
| **Built-in Tools** | Math, HTTP fetch, file I/O, datetime, web search, code execution | 7 proxy tools via `/tools/*` endpoints + 2 local vector tools                          |
...
| **Templates**   | Pre-built agent workflows                    | 5 workflow templates: agent-workflow, create-workflow, rag-ingest, scheduled-summary, web-research |
| **Integration** | Trigger agent runs from workflows            | HTTP nodes calling `agent-service` REST API                                                        |
...
| Tool Execution         | FastAPI sandboxed runtime                     | tools-service                                       | `main.py`                     |
